...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] createSampleConfiguration(): operation returned "
               "status code: 0x%08x \n",
               retStatus);
        goto CleanUp;
    }
    ...
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] readFrameFromDisk(): operation returned status code: "
               "0x%08x \n",
               retStatus);
        goto CleanUp;
    }
    ...
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] initKvsWebRtc(): operation returned status code: "
               "0x%08x \n",
               retStatus);
        goto CleanUp;
    }
    ...
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] createSignalingClientSync(): operation returned "
               "status code: 0x%08x \n",
               retStatus);
        goto CleanUp;
    }
    printf("[KVS Master] Signaling client created successfully\n");
    UINT64 delay = GETTIME();

    // Enable the processing of the messages
    retStatus = signalingClientConnectSync(pSampleConfiguration->signalingClientHandle);
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] signalingClientConnectSync(): operation returned "
               "status code: 0x%08x \n",
               retStatus);
        goto CleanUp;
    }
    delay = GETTIME() - delay;
    printf("DELAY: %lf seconds\n", (DOUBLE)(delay) / HUNDREDS_OF_NANOS_IN_A_SECOND);
    ...
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] sessionCleanupWait(): operation returned status code: "
               "0x%08x \n",
               retStatus);
        goto CleanUp;
    }
...
CleanUp:
    ...
    if (pSampleConfiguration != NULL) {
        ...
        if (retStatus == STATUS_SUCCESS) {
            ...
            printf("[KVS Master] signalingClientGetMetrics() operation returned "
                   "status code: 0x%08x",
                   retStatus);
        }
        retStatus = freeSignalingClient(&pSampleConfiguration->signalingClientHandle);
        if (retStatus != STATUS_SUCCESS) {
            printf("[KVS Master] freeSignalingClient(): operation returned status "
                   "code: 0x%08x",
                   retStatus);
        }
        ...
        if (retStatus != STATUS_SUCCESS) {
            printf("[KVS Master] freeSampleConfiguration(): operation returned "
                   "status code: 0x%08x",
                   retStatus);
        }
    }
...
PVOID sendVideoPackets(PVOID args)
{
    ...
    if (pSampleConfiguration == NULL) {
        printf("[KVS Master] sendVideoPackets(): operation returned status code: "
               "0x%08x \n",
               STATUS_NULL_ARG);
        goto CleanUp;
    }
    ...
    while (!ATOMIC_LOAD_BOOL(&pSampleConfiguration->appTerminateFlag)) {
        ...
        if (retStatus != STATUS_SUCCESS) {
            printf("[KVS Master] readFrameFromDisk(): operation returned status "
                   "code: 0x%08x \n",
                   retStatus);
            goto CleanUp;
        }
        ...
        if (frameSize > pSampleConfiguration->videoBufferSize) {
            ...
            if (pSampleConfiguration->pVideoFrameBuffer == NULL) {
                printf("[KVS Master] Video frame Buffer reallocation failed...%s (code "
                       "%d)\n",
        ...
        if (!ATOMIC_LOAD_BOOL(&pSampleConfiguration->updatingSampleStreamingSessionList)) {
            ...
            for (i = 0; i < pSampleConfiguration->streamingSessionCount; ++i) {
                ...
                encoderStats.encodeTimeMsec = 4; // update encode time to an arbitrary
...
PVOID sendAudioPackets(PVOID args)
{
    ...
    if (pSampleConfiguration == NULL) {
        printf("[KVS Master] sendAudioPackets(): operation returned status code: "
               "0x%08x \n",
               STATUS_NULL_ARG);
        goto CleanUp;
    }
    ...
    while (!ATOMIC_LOAD_BOOL(&pSampleConfiguration->appTerminateFlag)) {
        ...
        if (frameSize > pSampleConfiguration->audioBufferSize) {
            ...
            if (pSampleConfiguration->pAudioFrameBuffer == NULL) {
                printf("[KVS Master] Audio frame Buffer reallocation failed...%s (code "
                       "%d)\n",
                       strerror(errno), errno);
                printf("[KVS Master] realloc(): operation returned status code: 0x%08x \n", STATUS_NOT_ENOUGH_MEMORY);
                goto CleanUp;
...
PVOID sampleReceiveAudioFrame(PVOID args)
{
    ...
    if (pSampleStreamingSession == NULL) {
        printf("[KVS Master] sampleReceiveAudioFrame(): operation returned status "
               "code: 0x%08x \n",
               STATUS_NULL_ARG);
        goto CleanUp;
    }
    ...
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] transceiverOnFrame(): operation returned status code: "
               "0x%08x \n",
               retStatus);
        goto CleanUp;
    }
