...
Peer::Connection::Connection(PPeer pPeer, std::string id) : pPeer(pPeer), id(id)
{
}
...
STATUS Peer::connectSignaling()
{
    ...
    clientCallbacks.messageReceivedFn = [](UINT64 customData, PReceivedSignalingMessage pMsg) -> STATUS {
        ...
        auto it = std::find_if(pPeer->connections.begin(), pPeer->connections.end(), [&](const Connection& c) { return c.id == msgClientId; });

        if (it == pPeer->connections.end()) {
            pPeer->connections.emplace_back(Connection(pPeer, msgClientId));
            pConnection = &pPeer->connections[pPeer->connections.size() - 1];
        } else {
            pConnection = &(*it);
        }

        pConnection->handleSignalingMsg(pMsg);
